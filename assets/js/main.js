...
function startGame() {
    if ($('#name').val() != '') {
        ...
        var bgPosition = 9500; //размер фона
        ...
        var gamePaused = false; //состояние паузы
         ...
         */
        var opponents = [];
        var playerCurrentDirection = null; //это направление дял функции updatePlayer
        ...
        function gameEnd() {
            $('.panel').show(300);
            $('.screen-ranking').css("zIndex", "1000");
            $('.screen-ranking').css("width", "70%");
            $('#type').text("Победа!");
            ...
            $.ajax({
                ...
                success: function (msg) {
                    ...
                    for (var i = 0; i < 10; i++) {
                        num = i + 1;
                        $('#raiting tr:last').after(
                            '<tr>' +
                            '<th>' + num + '</th>' +
                            '<th>' + msg[i].username + '</th>' +
                            '<th>' + msg[i].score + '</th>' +
                            '<th>' + msg[i].time + '</th>' +
                            '</tr>');
                    }
